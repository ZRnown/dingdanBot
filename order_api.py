...
class OrderAPI:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        log(f"获取订单失败: HTTP {response.status_code}，{wait_time}秒后重试 (尝试 {attempt + 1}/{max_retries})")
                        time.sleep(wait_time)
                    else:
                        log(f"获取订单失败: HTTP {response.status_code}，已达到最大重试次数")
                        return None
            except Exception as e:
                if attempt < max_retries - 1:
                    wait_time = (attempt + 1) * 2
                    log(f"获取订单异常: {e}，{wait_time}秒后重试 (尝试 {attempt + 1}/{max_retries})")
                    time.sleep(wait_time)
                else:
                    log(f"获取订单异常: {e}，已达到最大重试次数")
                    return None
    ...
    def _get_shequ_orders(self, shequ_id: Optional[int], days: int, cutoff_date, today_date) -> List[Dict]:
        ...
        while True:
            ...
            if not result:
                log(f"[{shequ_name}] 获取第 {page} 页失败，已重试10次，跳过该页继续")
                page += 1
                if page > 100:
                    break
                continue
            ...
            if result.get('error') != 0:
                log(f"[{shequ_name}] 获取第 {page} 页返回错误: {result.get('error')}")
                page += 1
                if page > 100:
                    break
                continue
        ...
        log(f"[{shequ_name}] 获取完成，共 {len(orders)} 个订单")
        return orders
    ...
    def get_recent_orders(self, days: int = 2, shequ_ids: List[int] = None) -> List[Dict]:
        ...
        log(f"开始并行获取最近 {days} 天的订单（从 {cutoff_date} 到 {today_date}）...")
        ...
        with ThreadPoolExecutor(max_workers=min(len(shequ_ids), 10)) as executor:
            ...
            for future in as_completed(future_to_shequ):
                ...
                try:
                    ...
                    shequ_name = f"第三方 {shequ_id}" if shequ_id else "全部"
                    log(f"[{shequ_name}] 获取订单时发生异常: {e}")
        
        log(f"总共获取到 {len(all_orders)} 个最近 {days} 天的订单")
        return all_orders
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if result.get('error') == 0:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            log(f"订单 {order_id} 同步失败: {result}，{wait_minutes:.1f}分钟后重试 (尝试 {attempt + 1}/{max_retries})")
                            time.sleep(wait_seconds)
                        else:
                            log(f"订单 {order_id} 同步失败: {result}，已达到最大重试次数")
                            return {
                                'success': False,
                                'attempt': attempt + 1,
                                'refund_status': None,
                    ...
                    if attempt < max_retries - 1:
                        wait_seconds = random.randint(retry_interval_min * 60, retry_interval_max * 60)
                        wait_minutes = wait_seconds / 60
                        log(f"订单 {order_id} 同步失败: HTTP {response.status_code}，{wait_minutes:.1f}分钟后重试 (尝试 {attempt + 1}/{max_retries})")
                        time.sleep(wait_seconds)
                    else:
                        log(f"订单 {order_id} 同步失败: HTTP {response.status_code}，已达到最大重试次数")
                        return {
                            'success': False,
                            'attempt': attempt + 1,
                            'refund_status': None,
                ...
                if attempt < max_retries - 1:
                    wait_seconds = random.randint(retry_interval_min * 60, retry_interval_max * 60)
                    wait_minutes = wait_seconds / 60
                    log(f"同步订单 {order_id} 异常: {e}，{wait_minutes:.1f}分钟后重试 (尝试 {attempt + 1}/{max_retries})")
                    time.sleep(wait_seconds)
                else:
                    log(f"同步订单 {order_id} 异常: {e}，已达到最大重试次数")
                    return {
                        'success': False,
                        'attempt': attempt + 1,
                        'refund_status': None,
    ...
    def get_order_status_by_id(self, order_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                log(f"获取订单 {order_id} 状态失败: HTTP {response.status_code}")
        except Exception as e:
            log(f"获取订单 {order_id} 状态异常: {e}")
        return None
